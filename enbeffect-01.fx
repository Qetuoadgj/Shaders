...
// True Vision ENB (2015) - by Bronze316
...
#define POSTPROCESS					      2 //2	// Choose which post-processing effect to use 2.
#define APPLYGAMECOLORCORRECTION 	1 //1	// This will deactivate the use of Vanilla post-processing and only use ENB own post-processing.
#define ENB_FLIPTECHNIQUE 			  0 //0	// This will turn every effect in here off + SSAO, Reflections, Skylighting, Detailed Shadows, Sun Rays, ImageBasedLighting and Rain.
//
#define FLIP_INT_EXT				      0 //0	// Flips the interior and exterior factor, so that interior settings affect exteriors and vice versa.
#define FLIP_NIGHT_DAY				    0 //0	// Flips the day and night factor, so that day settings affect nights and vice versa.
//
#define ENB_ADAPTATION			      0 //1	// Enables ENB dynamic Adaptation settings.
#define HD6_ADAPTATION			      0 //1	// Enables HD6 static Skyrim Adaptation settings.
#define VANILLA_ADAPTATION		    1 //0	// Enables Skyrim Adaptation settings.
#define VANILLA_BLOOM			        1 //0	// Enables Skyrim Bloom settings.
//
#define HD6_BLOOM				          0 //1	// 1,2. 1 = Bloom Crisp - alternate crisp bloom, no hazey mud. 2 = Bloom Screen alternate bloom (using screen mode).
#define HD6_BLOOM_NOBLACK		      0 //0	// Brightens the dark spots of bloom.
#define HD6_BLOOM_DEFUZZ		      0 //0	// HD6 attempt to remove some of the haze from the bloom.
//
#define HD6_COLOR_TWEAKS		      0 //1	// Enables a set of Contrast, Brightness and Saturation controls, with in-game adjustment.
#define HD6_COLORSAT			        1 //1	// Adjusts the RGB values seperately, Red, Green and Blue + Magenta, Cyan and Yellow color
#define HD6_BLOOM_DEBLUEIFY		    0 //1	// With this enabled you can adjust the coloization of the bloom. Same settings as "KOLORIZER" but affects only bloom.
#define PALETTE_MIXER			        1 //1	// Enable settings to adjust the enbpalette.bmp
#define HD6_VIGNETTE			        1 //1
//
#define FIX_POW_ERROR             1 // Fix error when "pow(x, y = 1)" not equal "x"
#if (FIX_POW_ERROR==1)
  int pow_error = 1; // Always must be "1", DO NOT EDIT!
#endif
//

#if (POSTPROCESS==1)
//EXTERIOR									                 Night, Day
  float2	EContrastV1Ext				    = float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );	// Higher amounts make the dark areas darker while making the bright spot brighter.
  float2	EColorSaturationV1Ext		  = float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );	// Adds more color to the screen.
  float2	EToneMappingCurveV1Ext		= float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );	// Increasing this darkens the image and makes the bright spots less intense, Explanation of Tonemapping - http://en.wikipedia.org/wiki/Tone_mapping
  float2	EToneMappingOversatV1Ext	= float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );	// Similar to EToneMappingCurve but more subtle and makes the bright spots less intense and less colorful.

//INTERIOR									                 Night, Day
  float2	EContrastV1Int				    = float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );
  float2	EColorSaturationV1Int		  = float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );
  float2	EToneMappingCurveV1Int		= float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );
  float2	EToneMappingOversatV1Int	= float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );
#endif

#if (POSTPROCESS==2) // Modifications and tweaks by Bronze316
#define FactorV2 1.2
//EXTERIOR									                 Night, Day
  float2	EBrightnessV2Ext	        = float2( 1.2, 1.2 );	//float2( 1.4, 1.4 );	//float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );
  float2	EIntensityContrastV2Ext	  = float2( 1.2, 1.2 );	//float2( 1.2, 1.2 );	//float2( 1.0, 1.0 );	//float2( 1.4, 1.4 );
  float2	EColorSaturationV2Ext	    = float2( 1.4, 1.4 );	//float2( 1.4, 1.4 );	//float2( 1.0, 1.0 );	//float2( 1.633334, 1.866667 );
  float2	EToneMappingCurveV2Ext	  = float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );	//float2( 8.0, 8.0 );
  float2	EToneMappingOversatV2Ext  = float2( 4.0, 4.0 );	//float2( 8.0, 8.0 );	//float2( 1.0, 1.0 );	//float2( 1306.6, 2986.6 );
  float2	EBlacknessV2Ext		        = float2( 0.0, 0.0 );	//float2( 0.0, 0.0 );	//float2( 0.0, 0.0 );	//float2( 0.000001, 0.000001 );
  float2	EPostBrightnessV2Ext	    = float2( 1.2, 1.2 );	//float2( 1.4, 1.4 );	//float2( 1.0, 1.0 );	//float2( 0.816667, 0.933334 );

//INTERIOR									                 Night, Day       Night, Day
  float2	EBrightnessV2Int	        = float2( 1.4, 1.4 );	//float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );
  float2	EIntensityContrastV2Int	  = float2( 1.2, 1.2 );	//float2( 1.0, 1.0 );	//float2( 1.4, 1.4 );
  float2	EColorSaturationV2Int	    = float2( 1.4, 1.4 );	//float2( 1.0, 1.0 );	//float2( 1.633334, 1.633334 );
  float2	EToneMappingCurveV2Int	  = float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );	//float2( 8.0, 8.0 );
  float2	EToneMappingOversatV2Int  = float2( 8.0, 8.0 );	//float2( 1.0, 1.0 );	//float2( 2613.3, 2613.3 );
  float2	EBlacknessV2Int		        = float2( 0.0, 0.0 );	//float2( 0.0, 0.0 );	//float2( 0.000001, 0.000001 );
  float2	EPostBrightnessV2Int	    = float2( 1.4, 1.4 );	//float2( 1.0, 1.0 );	//float2( 1.225, 1.225 );
#endif

#if (POSTPROCESS==3)
//EXTERIOR									                Night, Day
  float2	EToneMappingCurveV3Ext  = float2( 1.0, 1.0 );	//float2( 4.0, 4.0 );	  // Increasing this darkens the image, Explanation of Tonemapping - http://en.wikipedia.org/wiki/Tone_mapping
  float2	EToneMapOversatV3Ext		= float2( 1.0, 1.0 );	//float2( 60.0, 60.0 );	// Increasing this darkens the image and makes the bright spots less intense, Explanation of Tonemapping - http://en.wikipedia.org/wiki/Tone_mapping

//INTERIOR									                Night, Day
  float2	EToneMappingCurveV3Int  = float2( 1.0, 1.0 );	//float2( 4.0, 4.0 );
  float2	EToneMapOversatV3Int		= float2( 1.0, 1.0 );	//float2( 60.0, 60.0 );
#endif

#if (POSTPROCESS==4)
//EXTERIOR									                 Night, Day
  float2	EBrightnessCurveV4Ext		  = float2( 0.7, 0.7 );	  //float2( 0.7, 0.7 );	  // A sort of contrast setting
  float2	EBrightMultiplierV4Ext	 	= float2( 0.45, 0.45 );	//float2( 0.45, 0.45 );	// A different kind of brightness setting
  float2	EBrightToneMapCurveV4Ext	= float2( 0.5, 0.5 );	  //float2( 0.5, 0.5 );	  // Behaves in similar ways as EToneMappingCurve

//INTERIOR									                 Night, Day
  float2	EBrightnessCurveV4Int		  = float2( 0.7, 0.7 );	  //float2( 0.7, 0.7 );
  float2	EBrightMultiplierV4Int		= float2( 0.45, 0.45 );	//float2( 0.45, 0.45 );
  float2	EBrightToneMapCurveV4Int	= float2( 0.5, 0.5 );	  //float2( 0.5, 0.5 );
#endif

#if (POSTPROCESS==5)
//EXTERIOR									               Night, Day
  float2	EBrightnessV5Ext			  = float2( 1.0, 1.0 );	//float2( 3.1, 1.5 );	    // Adjust the brightness of the scene.
  float2	EPostBrightnessV5Ext		= float2( 1.0, 1.0 );	//float2( 1.3, 1.3 );	    // Brightness that is rendered after the saturation and contrast. Compensates for darkening caused by those methods.
  float2	EIntensityContrastV5Ext	= float2( 1.0, 1.0 );	//float2( 3.475, 3.475 );	// Adjust the Contrast of the whole scene. Makes bright areas brighter and dark areas darker.
  float2	EColorSaturationV5Ext		= float2( 1.0, 1.0 );	//float2( 3.3, 3.3 );	    // Adjust the entire scene saturation. Adds more or less color if you increase or decrease the value
  float2	EToneMappingCurveV5Ext	= float2( 0.7, 0.7 );	//float2( 8.0, 8.0 );	    // A type of contrast that darkens everything, in simplest terms.

//INTERIOR									               Night, Day
  float2	EBrightnessV5Int			  = float2( 1.0, 1.0 );	//float2( 3.1, 1.5 );
  float2	EPostBrightnessV5Int		= float2( 1.0, 1.0 );	//float2( 1.3, 1.3 );
  float2	EIntensityContrastV5Int	= float2( 1.0, 1.0 );	//float2( 3.475, 3.475 );
  float2	EColorSaturationV5Int		= float2( 1.0, 1.0 );	//float2( 3.3, 3.3 );
  float2	EToneMappingCurveV5Int	= float2( 0.7, 0.7 );	//float2( 8.0, 8.0 );
#endif
...
#if (ENB_ADAPTATION==1)
//EXTERIOR									         Night, Day
	float2 EAdaptationMinExt 	= float2( 0.0, 0.0 );	//float2( 0.0, 0.0 );	//float2( 0.0, 0.0 );   // Determines the lowest amount the Adaptation will adjust the brightness.
	float2 EAdaptationMaxExt 	= float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );	//float2( 0.25, 0.25 ); // Determines the highest amount the Adaptation will adjust the brightness.

//INTERIOR									         Night, Day
	float2 EAdaptationMinInt	= float2( 0.0, 0.0 );	//float2( 0.0, 0.0 );	//float2( 0.0, 0.0 );
	float2 EAdaptationMaxInt	= float2( 1.0, 1.0 );	//float2( 0.5, 0.5 );	//float2( 0.25, 0.25 );
#endif

#if (HD6_ADAPTATION==1)
//EXTERIOR									           Night, Day
	float2 EAdaptationStaticExt = float2( 0.5, 0.5 );	//float2( 0.5, 0.5 );	// Adjust Skyrim min or max setting, range 0 to 1
	float2 EAdaptationCompExt 	= float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );	// Compensates for any loss in brightness.

//INTERIOR									           Night, Day
	float2 EAdaptationStaticInt	= float2( 0.5, 0.5 );	//float2( 0.5, 0.5 );
	float2 EAdaptationCompInt 	= float2( 1.0, 1.0 );	//float2( 1.0, 1.0 );
#endif

#if (VANILLA_ADAPTATION==1)
//EXTERIOR									                 Night, Day
  float2 fVanillaAdaptAmbientExt 		= float2( 1.0, 1.0 ); //float2( 0.5, -0.28 );	// Controls how much ambient luminosity there is from vanilla adaptation code.
  float2 fVanillaAdaptChangeExt  		= float2( 0.0, 0.0 ); //float2( 0.0, 0.0 );   // Controls how much the brightness changes when looking up and down. Affects ambient too.

//INTERIOR									                 Night, Day
  float2 fVanillaAdaptAmbientInt 		= float2( 1.0, 1.0 ); //float2( 1.0, 1.0 );
  float2 fVanillaAdaptChangeInt  		= float2( 0.0, 0.0 ); //float2( 0.0, 0.0 );
#endif

#if (VANILLA_BLOOM==1)
  //EXTERIOR									         Night, Day
  float2 fVanillaBloomExt			= float2( 1.0, 1.0 ); //float2( 1.0, 1.0 ); // Controls the intensity of vanilla bloom effect.
  float2 fVanillaBloomConExt	= float2( 0.0, 0.0 ); //float2( 0.0, 0.0 ); // This clamps the vanilla bloom to bright spots and darkens the dark areas.

  //INTERIOR									         Night, Day
  float2 fVanillaBloomInt			= float2( 1.0, 1.0 ); //float2( 1.0, 1.0 );
  float2 fVanillaBloomConInt	= float2( 0.0, 0.0 ); //float2( 0.0, 0.0 );
#endif
...
//EXTERIOR							               Night, Day
  float2 bBrightnessExt		    = float2( 1.0, 1.0 );	  //float2( 1.0, 1.0 );	  // Adjust bloom brightness.
  float2 bContrastExt			    = float2( 1.0, 1.0 );	  //float2( 1.1, 1.0 );	  // Adjust bloom contrast.
  float2 bSaturationExt		    = float2( 1.0, 1.0 );	  //float2( 0.8, 0.7 );   // Adjust bloom saturation.
  float2 bTriggerExt			    = float2( 1.0, 1.0 );	  //float2( 0.18, 0.0 );	// Darkens bloom and thus limits bloom to trigger.
  float2 bLimiterExt			    = float2( 1.0, 1.0 );	  //float2( 0.7, 0.7 );	  // Limits the amount of bloom.
  float2 bAddCompensateExt	  = float2( 0.0, 0.0 );	  //float2( 0.12, 0.23 );	// Compensate scene brightness by adding regular Brightness on top of the bloom.
  float2 bMultCompensateExt	  = float2( 1.0, 1.0 );	  //float2( 1.1, 1.4 );	  // Compensate scene brightness by multiplying regular Brightness on top of the bloom.
  float2 bBrtSpotStrengthExt	= float2( 1.0, 1.0 );	  //float2( 1.0, 1.0 );	  // Brightens only the super bright spots.
  float2 bBrtSpotStrength2Ext	= float2( 1.0, 1.0 );	  //float2( 1.0, 1.0 );	  // Brightens only bright spots.
  float2 bBlendOriginalExt	  = float2( 0.0, 0.0 );	  //float2( 0.7, 0.6 );	  // Blends in some additional bloom
  float2 bCompOriginalExt		  = float2( 1.0, 1.0 );	  //float2( 0.8, 0.7 );	  // Compensate scene brightness caused by BlendOriginal, multiplies regular scene Brightness.

//INTERIOR                             Night, Day
  float2 bBrightnessInt		    = float2( 1.0, 1.0 );	  //float2( 1.0, 1.0 );
  float2 bContrastInt			    = float2( 1.0, 1.0 );	  //float2( 1.1, 1.1 );
  float2 bSaturationInt		    = float2( 1.0, 1.0 );	  //float2( 0.8, 0.8 );
  float2 bTriggerInt			    = float2( 1.0, 1.0 );	  //float2( 0.18, 0.18 );
  float2 bLimiterInt			    = float2( 1.0, 1.0 );	  //float2( 0.7, 0.7 );
  float2 bAddCompensateInt	  = float2( 0.0, 0.0 );	  //float2( 0.12, 0.12 );
  float2 bMultCompensateInt	  = float2( 1.0, 1.0 );	  //float2( 1.1, 1.1 );
  float2 bBrtSpotStrengthInt	= float2( 1.0, 1.0 );	  //float2( 1.0, 1.0 );
  float2 bBrtSpotStrength2Int	= float2( 1.0, 1.0 );	  //float2( 1.0, 1.0 );
  float2 bBlendOriginalInt	  = float2( 0.0, 0.0 );	  //float2( 0.7, 0.7 );
  float2 bCompOriginalInt		  = float2( 1.0, 1.0 );	  //float2( 0.8, 0.8 );
#endif

#if (HD6_BLOOM==2)			// Bloom Screen
//EXTERIOR					            Night, Day
  float2 BloomMultExt	= float2( 10.0, 10.0 );	//float2( 10.0, 10.0 );	// Increase the intensity of the bloom

//INTERIOR					            Night, Day
  float2 BloomMultInt	= float2( 10.0, 10.0 );	//float2( 10.0, 10.0 );
#endif

#if (HD6_BLOOM_NOBLACK==1)
//EXTERIOR						               Night, Day
  float2 BloomBlacknessExt = float2( 0.12, 0.12 ); //float2( 0.12, 0.12 ); // Controls the amount of vanilla bloom effect.
//INTERIOR                           Night, Day
  float2 BloomBlacknessInt = float2( 0.12, 0.12 ); //float2( 0.12, 0.12 );
#endif

#if (HD6_BLOOM_DEFUZZ==1)
//EXTERIOR						Night, Day
  float2 DefuzzAlphaExt	= float2( 0.333, 0.333 );	//float2( 0.333, 0.333 );	// Adjust the bloom RGB Alpha channel.
  float2 DefuzzSubExt		= float2( 0.333, 0.333 );	//float2( 0.3, 0.3 );	    // Subtract the bloom intensity.
  float2 DefuzzAddExt		= float2( 0.333, 0.333 );	//float2( 0.22, 0.22 ); 	// Add bloom intensity.

//INTERIOR						Night, Day
  float2 DefuzzAlphaInt	= float2( 0.333, 0.333 );	//float2( 0.333, 0.333 );
  float2 DefuzzSubInt		= float2( 0.333, 0.333 );	//float2( 0.3, 0.3 );
  float2 DefuzzAddInt		= float2( 0.333, 0.333 );	//float2( 0.22, 0.22 );
#endif
...
//EXTERIOR							          Red, Green, Blue
  float3 ctRGBExtDay   		= float3( 1.0, 1.0, 1.0 );  //float3( 1.0, 1.0, 1.0 );	// RGB balance Day
  float3 ctRGBExtNight 		= float3( 1.0, 1.0, 1.0 );  //float3( 1.0, 1.0, 1.0 );	// RGB balance Night
  //									                  Night, Day
  float2 ctPreBrightnessExt 	= float2( 1.00, 1.00 ); //float2( 1.00, 1.00 );	    // Brightness applied before Contrast
  float2 ctPostBrightnessExt	= float2( 1.00, 1.00 ); //float2( 1.00, 1.00 );	    // Brightness applied after Contrast
  float2 ctContrastExt 		    = float2( 1.00, 1.00 ); //float2( 1.16, 1.305 );	  // Contrast
  float2 ctSaturationExt		  = float2( 1.00, 1.00 ); //float2( 0.875, 0.875 ); 	// Saturation
  float2 ctDarkenExt		      = float2( 0.00, 0.00 ); //float2( 0.0, 0.0 );	      // Saturation

//INTERIOR							           Red, Green, Blue
  float3 ctRGBIntDay   		= float3( 1.0, 1.0, 1.0 );  //float3( 1.0, 1.0, 1.0 );	// RGB balance Day
  float3 ctRGBIntNight 		= float3( 1.0, 1.0, 1.0 );  //float3( 1.0, 1.0, 1.0 );	// RGB balance Night
  //									                  Night, Day
  float2 ctPreBrightnessInt 	= float2( 1.00, 1.00 ); //float2( 1.00, 1.00 );	    // Brightness applied before Contrast
  float2 ctPostBrightnessInt	= float2( 1.00, 1.00 ); //float2( 1.00, 1.00 );	    // Brightness applied after Contrast
  float2 ctContrastInt 		    = float2( 1.00, 1.00 ); //float2( 1.22, 1.22 );	    // Contrast
  float2 ctSaturationInt		  = float2( 1.00, 1.00 ); //float2( 0.875, 0.875 );	  // Saturation
  float2 ctDarkenInt		      = float2( 0.00, 0.00 ); //float2( 0.0, 0.0 );	      // Saturation
#endif

#if (HD6_COLORSAT==1)
//EXTERIOR						           Red, Green, Blue
  float3 RGBSatExtDay 	= float3( 1.15, 1.05, 1.05 );  //float3( 1, 1, 1 );  //float3( 1, 1, 1 );
  float3 RGBSatExtNight = float3( 1.15, 1.05, 1.05 );  //float3( 1, 1, 1 );  //float3( 1.0625, 0.96875, 0.96875 );

//INTERIOR						           Red, Green, Blue
  float3 RGBSatIntDay 	= float3( 1, 1, 1 );  //float3( 1, 1, 1 );  //float3( 1, 0.975, 1.025 );
  float3 RGBSatIntNight = float3( 1, 1, 1 );  //float3( 1, 1, 1 );  //float3( 1, 0.975, 1.025 );
#endif

#if (HD6_BLOOM_DEBLUEIFY==1)
//EXTERIOR						                  Red, Green, Blue
  float3 DebluifyExtDay 	= float3( 1, 1, 1 );  //float3( 1.0625, 1.0625, 1.0625 );
  float3 DebluifyExtNight = float3( 1, 1, 1 );  //float3( 1.128906, 1.029297, 1.029297 );

//INTERIOR						                  Red, Green, Blue
  float3 DebluifyIntDay 	= float3( 1, 1, 1 );  //float3( 1.0625, 1.035938, 1.089063 );
  float3 DebluifyIntNight = float3( 1, 1, 1 );  //float3( 1.0625, 1.035938, 1.089063 );
#endif

#if (PALETTE_MIXER==1)
//EXTERIOR							          Night, Day
  float2 palmixExt 		  = float2( 1.0, 1.0 ); //float2( 1.05, 1.05 );
  float2 PaletteMinExt 	= float2( 1.0, 1.0 ); //float2( 1.05, 1.05 );
  float2 PaletteMaxExt 	= float2( 1.0, 1.0 ); //float2( 1.05, 1.05 );

//INTERIOR							          Night, Day
  float2 palmixInt		  = float2( 1.0, 1.0 ); //float2( 1.05, 1.05 );
  float2 PaletteMinInt 	= float2( 1.0, 1.0 ); //float2( 1.05, 1.05 );
  float2 PaletteMaxInt 	= float2( 1.0, 1.0 ); //float2( 1.05, 1.05 );
#endif
...
#if (HD6_VIGNETTE==1)
//EXTERIOR							               Night, Day
  float2 VignetteRoundExt		  = float2( 0.4, 0.4 );   //float2( 0.4, 0.4 );   // Determines how round the vignette should be.
  float2 VignetteSquareTopExt	= float2( 0, 0 );       //float2( 0, 0 );       // Determines how square the vignette should be. Adjusts the top of the screen
  float2 VignetteSquareBotExt	= float2( 0.1, 0.1 );   //float2( 0.1, 0.1 );   // Determines how square the vignette should be. Adjusts the bottom of the screen
  float2 VignetteSatExt		    = float2( 0.85, 0.85 ); //float2( 0.85, 0.85 ); // Determines how saturated the vignette should be.
  float2 VignetteContrastExt	= float2( 1.5, 1.5 );   //float2( 1.5, 1.5 );   // Determines how much contrast the vignette should have.
  float2 VignetteStrengthExt	= float2( 0.85, 0.85 ); //float2( 0.85, 0.85 ); // Determines how strong the vignette should be depending on time of day.

//INTERIOR							               Night, Day
  float2 VignetteRoundInt		  = float2( 0.4, 0.4 );   //float2( 0.4, 0.4 );
  float2 VignetteSquareTopInt	= float2( 0, 0 );       //float2( 0, 0 );
  float2 VignetteSquareBotInt	= float2( 0.1, 0.1 );   //float2( 0.1, 0.1 );
  float2 VignetteSatInt		    = float2( 0.85, 0.85 ); //float2( 0.85, 0.85 );
  float2 VignetteContrastInt	= float2( 1.5, 1.5 );   //float2( 1.5, 1.5 );
  float2 VignetteStrengthInt	= float2( 0.85, 0.85 ); //float2( 0.85, 0.85 );
#endif

//

float4 ColorSaturate( float4 colorInput, float satValue )
{
  //calculate monochrome
  float3 grey = dot(float3(0.21,0.72,0.07), colorInput.rgb);
	
  //Add back some of the color?
  colorInput.rgb = lerp(grey, colorInput.rgb, satValue); //Adjust the remaining saturation.
...
// Flip Code for DN-IE, now with interpolation
  ...
  float	grayadaptation=max(max(Adaptation.x, Adaptation.y), Adaptation.z);
  ...
  #else // Enabled Bloom for enbseries.ini controls
    ...
    #if (FIX_POW_ERROR==1)
      float3 scl_error=scl - pow(scl, pow_error);
      float3 xncol_error=xncol - pow(xncol, pow_error);

      scl=pow(scl, EIntensityContrastV2) + scl_error;
      xncol.xyz=pow(xncol.xyz, EColorSaturationV2) + xncol_error;
    #else
